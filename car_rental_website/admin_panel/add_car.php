...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if(isset($_FILES['image'])){
    ...
    if (empty(trim($_POST["brand"])) || empty(trim($_POST["colour"])) || empty(trim($_POST["d_price"])) || empty(trim($_POST["a_price"])) || empty(trim($_POST["desc"])) || $file_size <=0) {
        echo "The fields are empty";
    } else {
        // Database entry add a car database
        $sql = "INSERT INTO car_db (brand, colour, discounted_price, actual_price, descr,  file__name) VALUES ('$brand', '$colour','$d_price','$a_price', '$descr', '$file_name')";
...
<h1 align="center" style="font-size: 1.9rem;">Add a new car</h1>
...
<div class="container">
    <form method="post"  class="form_body" enctype="multipart/form-data">
        ...
        <input name="colour" id="colour" type="text">  


        <label for="d_price">Enter the discounted car price per day</label>
        <input name="d_price" id="d_price" type="number"style="height: 3rem; border:solid">  


        <label for="a_price">Enter the actual price per day</label>
        <input name="a_price" id="a_price" type="number" style="height: 3rem; border:solid">  <br><br>
